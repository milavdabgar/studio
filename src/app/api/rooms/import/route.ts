...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const row of parsedData) {
      ...
      // CCTV Information
      const cctvInstalled = csvRow.cctvinstalled !== undefined ? csvRow.cctvinstalled === true || csvRow.cctvinstalled === 'true' || csvRow.cctvinstalled === '1' : false;
      ...
      const cctvStatus = cctvStatusRaw && ['working', 'down', 'maintenance', 'not_installed'].includes(cctvStatusRaw) ? cctvStatusRaw as 'working' | 'down' | 'maintenance' | 'not_installed' : 'not_installed';
      ...
      const roomData: Partial<Room> = {
        ...
        cctv: {
          installed: cctvInstalled,
          company: cctvCompany,
          deviceNo: cctvDeviceNo,
          ipAddress: cctvIpAddress,
