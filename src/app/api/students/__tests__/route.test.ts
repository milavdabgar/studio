  ...
  describe('POST /api/students', () => {
    ...
    it('should use default domain when institute not found', async () => {
      ...
      mockStudentInstance.toJSON.mockReturnValue({
        ...validStudentData,
        id: 'student_125',
        instituteEmail: 'GP24CE001@gppalanpur.in',
        instituteId: 'inst_gpp',
        userId: 'user_123',
        createdAt: expect.any(String),
        updatedAt: expect.any(String)
      ...
      expect(response.status).toBe(201);
      expect(data.instituteEmail).toBe('GP24CE001@gppalanpur.in'); // Default domain
    });
    ...
    it('should handle program without instituteId', async () => {
      ...
      expect(response.status).toBe(201);
      expect(data.instituteEmail).toBe('GP24CE001@gppalanpur.in'); // Default domain used
    });
