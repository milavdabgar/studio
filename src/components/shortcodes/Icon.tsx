// src/components/shortcodes/Icon.tsx
"use client";
...
// Map Blowfish icon names to Heroicons
const iconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  // Default Blowfish icons
  'check': CheckIcon,
  'triangle-exclamation': ExclamationTriangleIcon,
...
export function Icon({ name, className = "w-5 h-5", size }: IconProps) {
  ...
  const finalClassName = size 
    ? `w-${size} h-${size} ${className.replace(/w-\d+|h-\d+/g, '')}`.trim()
    : className;
